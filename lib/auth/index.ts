...
export async function verifyCredentials(username: string, password: string): Promise<AdminUser | null> {
  ...
  `;
  if (!user) {
    await bcrypt.compare(password, "$2b$12$C6UzMDM.H6dfI/f/IKcEe.4a6s17VOFzQnwNJJuJN4qeY2rP6nO1O");
    return null;
  }
...
// Shared database counter works across all Vercel function instances.
export async function allowLogin(request: Request, username: string) {
  const ip = request.headers.get("x-vercel-forwarded-for")?.split(",")[0]?.trim()
    || request.headers.get("x-forwarded-for")?.split(",")[0]?.trim() || "local";
  const keys = ["ip:" + ip, "user:" + username.trim().toLowerCase()];
  const results = [];
  for (const key of keys) {
    const [row] = await sql<{ attempts: number }[]>`
      insert into admin_login_attempts (key) values (${hashToken(key)})
      ...
      returning attempts
    `;
    results.push(row.attempts);
  }
  await sql`delete from admin_login_attempts where window_start < now() - interval '1 day'`;
  return results[0] <= 30 && results[1] <= 10;
}
