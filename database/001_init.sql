...
-- Every table is server-only. No anon/authenticated access through the Data API.
...
begin
  if exists (select 1 from pg_roles where rolname = 'anon') then
    revoke all on categories, products, product_price_options, admin_users, admin_sessions, app_settings, admin_login_attempts from anon;
  end if;
  if exists (select 1 from pg_roles where rolname = 'authenticated') then
    revoke all on categories, products, product_price_options, admin_users, admin_sessions, app_settings, admin_login_attempts from authenticated;
  end if;
end $$;
revoke all on categories, products, product_price_options, admin_users, admin_sessions, app_settings, admin_login_attempts from public;
